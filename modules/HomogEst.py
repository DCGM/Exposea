...
def resize_fragment(img_frag: torch.Tensor,
        scale_x: float,
        scale_y: float,
):
...
def adjust_fragment(fragment, scale):
    """
...
class MatcherBuiltInLG:
    def __init__(self, logger, resize, config):
        self.logger = logger
        self.resize = resize
        self.config = config
        ...
        self.ref_img = None
        self.frag_img = None
        self.extractor = SuperPoint(max_num_keypoints=self.config.homog.max_feat_points).eval().to(self.device)
    ...
    def load_ref(self, ref_path):
        ref_img = load_image(ref_path)
        if self.resize is not None:
            ref_img, self.scale_x, self.scale_y = compute_scale_and_resize_ref(ref_img, max_res=self.resize)
        self.ref_img = ref_img
        self.ref_features = self.extractor.extract(ref_img.to(self.device))
        self.logger.info(f"REF Num. Features: {self.ref_features['keypoints'].shape[1]}")

    def match(self, frag_path, adjust_scale=0):
        if self.ref_img is None:
            self.logger.error("Homog | First initialize ref using load_ref()")
            raise AttributeError
        frag_img = load_img(frag_path)
        frag_name,_ = os.path.splitext(os.path.basename(frag_path))
        # Try to match details according to specified values in donfig
        frag_img = self.match_details(frag_img, adjust_scale)

        if self.resize is not None:
            frag_img = resize_fragment(frag_img, self.scale_x, self.scale_y)

        self.frag_img = frag_img
        frag_feats = self.extractor.extract(frag_img.to(self.device))

        # Match features with reference
        matches_a_b = self.matcher({"image0": self.ref_features, "image1": frag_feats})

        self.logger.info(
            f"[{frag_name}] Num. Features: {frag_feats['keypoints'].shape[1]} | Matches {matches_a_b['matches'][0].shape[0]}")

        # Reshape the input
        feats1, feats2, matches12 = [
            rbd(x) for x in [self.ref_features, frag_feats, matches_a_b]
        ]  # remove batch dimension
        # Extract Matched key points from key points lists for homography
        kpts1, kpts2, matches = feats1["keypoints"].cpu().numpy(), feats2["keypoints"].cpu().numpy(), matches12["matches"].cpu().numpy()
        m_kpts1, m_kpts2 = kpts1[matches[..., 0]], kpts2[matches[..., 1]]


        _save_debug_imgs([np.asarray(self.ref_img.permute(1, 2, 0).cpu()), m_kpts1, kpts1],
                        [np.asarray(self.frag_img.permute(1, 2, 0).cpu()), m_kpts2, kpts2],
                         matches12,
                         path=f"./plots/matches_{frag_name}.jpeg")

        if self.resize is not None:
            m_kpts1 = restore_kpts_to_original(m_kpts1, self.scale_x, self.scale_y)
            m_kpts2 = restore_kpts_to_original(m_kpts2, self.scale_x, self.scale_y)

        return [m_kpts2, m_kpts1]
    ...
    def match_details(self, frag_img, adjust_scale=0):
        ...
        if hasattr(self.config, "relative_scale"):
            try:
                ...
                frag_img = adjust_fragment(frag_img, scale)
            except:
...
class MatcherVisMatch:
    def __init__(self, logger, resize, config):
        ...
        if self.config.homog.feature_matcher in available_models:
            self.kornia = True
    ...
    def match(self, frag_path, adjust_scale=0):
        ...
        if self.resize is not None:
            frag_img = resize_fragment(frag_img, self.scale_x, self.scale_y)
            self.frag_img = frag_img
            results = self.matcher(frag_img, self.ref_img)
            if self.debug:
                plot_matches(self.frag_img, self.ref_img, results, save_path=f"./plots/matches_{frag_name}.jpeg")
            kpts_ref = restore_kpts_to_original(results["inlier_kpts1"], self.scale_x, self.scale_y)
            kpts_frag = restore_kpts_to_original(results["inlier_kpts0"], self.scale_x, self.scale_y)
...
class HomogEstimator:
    ...
    def __init__(self, config):
        ...
        self.scale_modifier = [1, -1, 2, -2, 4, -4]
        ...
        self.scale_x = 1.0
        self.scale_y = 1.0
        # Scale modifier in case of Autofix rematching
        self.scale_modifier = [1, -1, 2, -2, 4, -4]
        if self.config.homog.feature_matcher == 'LightGlue':
            self.matcher = MatcherBuiltInLG(self.logger,
                                            self.resize,
                                            self.config)
        else:
            self.matcher = MatcherVisMatch(self.logger,
                                           self.resize,
                                           self.config)
    ...
    def register(self, ref_path: str, frag_paths: list[str]):
        ...
        self.matcher.load_ref(ref_path)
        ...
        for idx, frag_path in enumerate(frag_paths):
            ...
            if H is None or kpts_frag.shape[0] < self.config.homog.min_matches or kpts_ref.shape[0] < self.config.homog.min_matches:
                self.logger.warning(f"Autofix | Matcher was unable to estimate homography trying simple autofix")

                for scale_modifier in self.scale_modifier:
                    kpts_frag, kpts_ref = self.matcher.match(frag_path, scale_modifier)
                    if kpts_frag.shape[0] > self.config.homog.min_matches or kpts_ref.shape[0] > self.config.homog.min_matches:
                        self.logger.warning(
                            f"Autofix | Succesfull with scale modifier {scale_modifier}")
                        H, mask = cv.findHomography(
                            kpts_frag,
                            kpts_ref,
                            cv.USAC_MAGSAC,
                        ...
                        H = None
            self.logger.info(
                f"[{idx}] Num. Matches {kpts_frag.shape[0]}")
            if H is None:
                self.logger.warning(f"Homog | Unable to find homography to fragment {frag_path}")
                to_skip.append(idx)
            homographies.append(H)
            corrs.append((kpts_frag, kpts_ref))
            ...
            #         if results is None:
            #             self.logger.warning(f"Autofix | Failed")
            #         kpts_ref = results["inlier_kpts1"]
            #         kpts_frag = results["inlier_kpts0"]
            #         mkpts = [kpts_frag, kpts_ref]
            #         self.logger.warning(f"Autofix | Found {mkpts[0].shape[0]} kpts")
            #         H, mask = cv.findHomography(
            #             kpts_frag,
            #             kpts_ref,
            #             cv.USAC_MAGSAC,
            ...
            # homographies.append(H)
            # corrs.append(mkpts)
        return homographies, corrs, to_skip
